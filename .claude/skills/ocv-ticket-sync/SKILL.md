...
`scripts/ado_sync.py execute` without `--yes`, it prints the same
...
- `--work-item-type "Bug"`
- `--owners-config configs/ado_owners_outlook-agent.json` — owner-routing
...
- Routing rules live in `configs/ado_owners_outlook-agent.json`
...
python scripts/ado_sync.py assign-owners \
  --subtopics data/ocv_<area>_<to>_subtopics.csv \
  --dry-run
...
- `scripts/ado_sync.py` — the script (propose / execute / assign-owners
  subcommands)
- `configs/ado_owners_outlook-agent.json` — owner-routing rules used by
